...
**No comments** — not in code, not in the jsonc data files, not in config or shell. A comment states what was true when it was typed; the code states what is true now, and the two drift apart silently. Explain with a clearer name, a smaller function, a test that demonstrates the behaviour, or prose in `docs/` that a reader knows to distrust. Directives the toolchain reads are not comments and stay: `eslint-disable*`, `svelte-ignore`, `@ts-expect-error`/`@ts-ignore`, `/// <reference …>`, `shellcheck disable`, and typed JSDoc (`@type`, `@param {…}`) in `.js` files, which the checker validates against the code.
