...
- [~] **Forage spawn balance + berry recolor (2026-06-19).** Rebalanced wild-forage distribution so the
  forageable seed sources read by biome: mushrooms boosted on the forest floor (tree/deep_grass), wild
  barley/rye boosted on plains (savanna/tall_grass/grass), and berry bushes ~halved (they over-spawned).
  Berries also recolored from a vivid `#BF2E38` to a muted dusky `#8C454F` so they stop dominating the
  palette. **NOT done:** adding new forage *variants* — tracked as a separate content task.
...
- [x] **Grove-glow legibility pass (2026-06-19):** brightened + saturated the ironwood/heartwood grove
  glows (intensity 0.40/0.45 → 0.52) so they read against their near-black glyphs, and recolored
  ironwood green → steel-blue (distinct from moonwood's paler blue). (Needs an in-game eyeball to confirm.)
- [x] **Crystal-node glow (2026-06-19):** all 7 crystal nodes now emit a faint per-gem-hue point light
  (intensity 0.08), routed through the same `LightingService.collectResourceEmitters` path as the groves,
  so gem nodes catch the eye without lighting the area like a fire.
