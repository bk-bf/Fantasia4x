...
| §4 Clay & Ceramics                     | ✅     | **Data ✅**: `fired_brick`, `water_urn` (container), `clay_mold` + `tile_mold`, `clay_roof_tile`; `pottery_kiln` building (`minFuelHeat: 2`). `clay_lump`/`mud_brick` already existed. **Pending**: `minFuelHeat` enforcement (fuel system tracks unit count, not fuel *type/heat* — needs typed-fuel tracking) + reusable-mold gating (cast in §5)                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| §5 Ore → Metal                         | ✅     | **Data ✅**: named minerals (malachite/chalcopyrite/azurite→copper, cassiterite→tin, hematite/magnetite/limonite→iron, galena→lead/silver, native_gold/electrum→gold) replace generic ore-node yields (resolved 3 `_TODO`s); bars `copper`/`tin`/`bronze`/`iron`/`lead`/`silver`/`gold`/`steel` with DF-style many-ore recipes + limestone flux; `stone_forge`/`bloomery`/`anvil` buildings; copper/tin nodes lowered to tier-1 (bronze before iron). **Pending**: fuel/flux *enforcement* + clay-mold consumption (logic, ties to minFuelHeat work)                                                                                                                                                                                                                              |
...
| §D Water needs                         | ✅     | **Core ✅ + tested**: `thirst`/`hygiene` accrue each tick in `processNeedsTick` (thirst 0.7/s, hygiene 0.3/s); parched (>90)/filthy (>85) pawns lose mood; **meals quench thirst** (relief without a no-op penalty); `water` item + `well` (Draw Well) building. **Pending (Stage 4)**: drinking/washing **AI** (pathfind to water/urn/well, Drinking/Washing states), dehydration→collapse condition, drink/wash-zone routing                                                                                                                                                                                                                                                                                                                                                       |
...
| §F Storage & roofs                     | ✅     | **Data ✅ + capacity live**: `hay_store`/`log_store`/`clay_cellar` raise their tile's capacity via `tileCapacityBonus` (functional — Stage 2) and carry `storageDecayMultiplier`; roof tiles `thatch_roof`/`plank_roof`/`tile_roof` (thatch decays fast, tile lasts). Durable goods already halt deterioration when stored (§B). **Pending (Stage 4)**: wire `storageDecayMultiplier` into organic spoilage; enclosure (walls+roof+door) detection                                                                                                                                                                                                                                                                                                                                  |
| §G Shelter & Light                     | ✅     | **Data ✅**: `thatch/plank/tile_roof` (+`transparentRoof` on tile), `door`, `window` build tiles; `animal_fat` (rendered from fatty meat), `torch`, `candle` light items. **Pending (Stage 4)**: crafting **light penalty** (read LightingService), window-daylight/roof-shade in the light model, enclosure gating                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
