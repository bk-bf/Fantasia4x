...
- [x] **Soil exhaustion + crop death (2026-06-20).** Each crop has a `fertilityCost`; a **harvested** crop adds it to `tile.fertilityWear` (new persisted field, /25 per tier) and at 25 the tile drops one soil tier (**terra preta → rich → loam → poor → barren dirt**, via `SUBTYPE_BY_SOIL_TIER`) — over-farming exhausts a plot over a realistic run (~8 wheat or ~3 pumpkin harvests per tier; prize crops draw fastest). **Only reaped crops wear soil.** A crop that **dies** — exhausted fertility (< `minSoil`), frost (`tile.snow > 0`), cold/heat (temp window), drought/flood (moisture window), or **grazing** (crops are now `grazing: true`; an animal eats an unprotected field) — is set to **1%** (not 0%), so it never reads as a harvested cycle and the map can't churn itself barren. Light is a non-lethal stall. Wild grass is unaffected (no `crop` spec → unconditional regrowth → effectively the lowest-requirement plant, so its map coverage holds).
