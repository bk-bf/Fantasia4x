...
| 2c | **Engine Performance & Scaling** — sim→Worker + soft-body pathfinding + **W2/W2b slim snapshot** (the win: `post` 31.6→6.5%, heavy stress case **80–100 TPS @4×**, FPS 60–80). De-immutabling + caches landed; Rust-SoA evaluated+aborted. Custom profiler retired → browser-native. Next (deferred): de-immutable residual pawn-patch spreads; LoS (P3) | [x] core perf done | [ENGINE-PERFORMANCE.md](ENGINE-PERFORMANCE.md) §B/§10 · ADR-021                                       |
