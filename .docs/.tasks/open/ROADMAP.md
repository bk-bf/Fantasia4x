...
| **Trait §0 cleanup + Culture rename** (traits are pure granters — resistances→`resistances` block, part-granted effects for grafts, organic scarring, condition self-triggers; `Race`→`Culture` throughout code+UI) | [-] 2026-07-09 | [TRAITS.md](TRAITS.md) §0 + [LINEAGES.md](LINEAGES.md); scar system + resistance sweep + Spider Eyes + rename done (`check` 0 err, 868/869); procs/weaponBonus/registry-bans remain |
| **Lineages** (ancestral-blood mutation trees: born/awaken via deed meters, grow at seasonal growth events; Beast/Werewolf/Vampiric/Amphibian/Arachnid + flattened heritages) | [ ] planned | [LINEAGES.md](LINEAGES.md) — supersedes the nested-`subCapabilities` heritage bundle; Phase 0 = finish §0, then foundation → Beast slice → Werewolf/Vampiric transform+feeding |
