...
| [PawnStateMachine.ts](../src/lib/game/systems/PawnStateMachine.ts) | ~~2,710~~ → **988** | ✅ **DECOMPOSED 2026-06-13** (was the #1 hotspot). Split into `pawn/handlers/{work,needs,combat}.ts` + shared `pawn/pawnHelpers.ts` + `pawn/pawnQueries.ts` + `pawn/pawnStates.ts`; what remains is the health/lifecycle block + a thin `Record<PawnState,Handler>` dispatcher (`tickPawn` fan-out 16 → ~1). Acyclic; 0 errors / 149 tests. Largest residual unit is `pawnHelpers.ts` (1,031) — a candidate for a further sub-split. See the [hotspot report](HOTSPOT-PawnStateMachine-2026-06-13.md) |
