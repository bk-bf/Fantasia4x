...
describe('steel chain', () => {
  ...
  it('every step of the chain QUEUES and PRODUCES', () => {
    ...
    for (const id of CHAIN) {
      ...
      const bld = (state.buildings ?? []).find(
        (b) => (b as { type?: string }).type === r.station
      ) as { id?: string };
      const order = {
        id: 'o1',
        item: { id: out, name: 'x', amount: 0 },
        quantity: 1,
  ...
  it('both irons share category:iron and each has exactly ONE producer (no shadow)', () => {
    expect(
      itemService
        .getItemsByCategory('iron')
        .map((i) => i.id)
        .sort()
    ).toEqual([...IRONS].sort());
    for (const [item, recipe] of [
      ['iron_bar', 'make_iron_bar'],
      ['wrought_iron', 'refine_wrought_iron']
    ] as const)
  ...
  it('the 6 steels all satisfy a category:steel consumer (any steel crafts a steel item)', () => {
    ...
    for (const s of steels) {
      const order = {
        id: 'o1',
        item: { id: 'steel_longsword', name: 'x', amount: 0 },
        quantity: 1,
        workRequired: 8,
        workDone: 0,
        inputs: {},
        stationType: 'anvil',
        stationBuildingId: 'b0'
      } as never;
      const gs = completeCraftOrder(
        order,
        {
          droppedItems: [
            { id: 'd1', resourceId: s, reservedFor: 'o1', quantity: 2, x: 0, y: 0, stored: false }
          ],
          craftingQueue: [order],
          buildings: [{ id: 'b0', type: 'anvil', x: 0, y: 0, status: 'complete' }],
          zoneTiles: {}
  ...
  it('pawns physically smelt iron at the bloomery and bake blister steel at the cementation furnace', async () => {
    ...
    for (let i = 0; i < 16 && !(stk().blister_steel > 0 && (stk().hematite ?? 0) < ore0); i++)
      s.tick(400);
    console.log(
      `[STEEL-PIPELINE] hematite ${ore0}→${stk().hematite}, iron_bar=${stk().iron_bar}, blister_steel=${stk().blister_steel}, turn=${s.getState().turn}`
    );
  ...
  it('pawns physically forge shear steel and pattern-welded steel at the anvil', async () => {
    ...
    s.command({
      type: 'craftItem',
      payload: { itemId: 'pattern_welded_steel', quantity: 1 }
    } as never);
