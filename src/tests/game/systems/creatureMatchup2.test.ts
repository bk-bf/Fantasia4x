...
/**
 * WEAPON x CREATURE, shard 2 of 8 — see `creatureMatchupHarness.ts` for the design.
 *
 ...
 */
describe('WEAPON x CREATURE — shard 2', () => {
  it('every weapon against this shard of the real hostile creatures', async () => {
    const rows = await runShard(2);
    const creatures = shardOf(2);
    const best = rows.slice().sort((a, b) => b.effectPer1k - a.effectPer1k).slice(0, 8);
    console.log(
      `[CREATURES shard 2] ${creatures.length} creatures x ${WEAPONS.length} weapons x ${SEEDS.length} fights\n` +
