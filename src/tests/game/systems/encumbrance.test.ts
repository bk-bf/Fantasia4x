...
// Worn armour + pack weight over a STRENGTH-scaled capacity drives the staged `encumbered` condition (set directly from the ratio).
describe('driveEncumbrance (load → staged condition)', () => {
  ...
  it('the overloaded stage cuts combat (DEXTERITY → dodge + aim) and movement', () => {
    ...
    // Evasion + aim flow through the DEXTERITY penalty; movement is its own modifier.
