...
/**
 * WEAPON x CREATURE, shard 5 of 8 — see `creatureMatchupHarness.ts` for the design.
 *
 ...
 */
describe('WEAPON x CREATURE — shard 5', () => {
  it('every weapon against this shard of the real hostile creatures', async () => {
    const rows = await runShard(5);
    const creatures = shardOf(5);
    const best = rows.slice().sort((a, b) => b.effectPer1k - a.effectPer1k).slice(0, 8);
    console.log(
      `[CREATURES shard 5] ${creatures.length} creatures x ${WEAPONS.length} weapons x ${SEEDS.length} fights\n` +
