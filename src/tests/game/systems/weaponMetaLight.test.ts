...
/**
 * ARMOUR SWEEP — target wearing LIGHT, attacker always naked.
 *
 ...
 *
 * Writes its ranking to `.debug/weapon-meta-light.json` so the cross-class movement table (which
...
describe(`WEAPON META — target in light armour`, () => {
  it('every naked attacker against every style', async () => {
        ...
        for (const seed of SEEDS) {
          ...
          if (++done % 50 === 0 || done === total)
            try {
              appendFileSync(
                PROGRESS,
                `  [light] ${done} of ${total} (${((done / total) * 100).toFixed(1)}%)\n`
              );
            } catch {
    ...
    console.log(
      `[ARMOUR SWEEP · target in LIGHT] attacker always naked, ${fights} fights each\n` +
        ranked
          .map(
            (r) =>
