...
/**
 * HEAD TO HEAD — every style against every other, nobody armoured, as a matchup GRID rather than a
 * single win total.
 *
 * A style with a poor overall record can still be a COUNTER: if most of what it wins comes from one or
 * two specific opponents, that is a build with a purpose, not a weak build. Twin daggers are the case
 * this exists to catch.
 *
 * Its own file so it runs alongside the four armour sweeps instead of queueing behind them — see
 * `weaponMetaHarness.ts` for why that matters.
 */
describe('WEAPON META — head to head, nobody armoured', () => {
  it('the full matchup grid, and who counters whom', async () => {
    ...
    let done = 0;
    const tick = () => {
      // Vitest buffers console output until a test ENDS, so disk is the only live progress channel.
