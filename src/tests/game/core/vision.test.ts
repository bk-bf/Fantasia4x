...
describe('isWitnessedByColony', () => {
  const DAY = 1.0;
  const range = effectiveVisionRange(atPawn(0, 0), DAY);

  it('an event within a pawn’s sight range is witnessed; one beyond it is not', () => {
    expect(isWitnessedByColony([atPawn(0, 0)], range, 0, DAY)).toBe(true);
    expect(isWitnessedByColony([atPawn(0, 0)], range + 1, 0, DAY)).toBe(false);
  });

  it('uses Chebyshev (diagonal counts as one); ANY pawn seeing it is enough', () => {
    expect(isWitnessedByColony([atPawn(0, 0)], range, range, DAY)).toBe(true);
