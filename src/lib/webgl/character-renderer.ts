...
export class CharacterRenderer {
	...
	constructor(gl: WebGL2RenderingContext, shaderManager: ShaderManager, fontAtlas: FontAtlas, debug = false) {
		this.gl = gl;
	...
	createSingleCharacter(options: CharacterRenderOptions): boolean {
		...
		const vertices = new Float32Array([
			// Triangle 1
			x1, y1, u1, v1, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],
			x2, y1, u2, v1, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],
			x1, y2, u1, v2, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],

			// Triangle 2
			x2, y1, u2, v1, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],
			x2, y2, u2, v2, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],
			x1, y2, u1, v2, foregroundColor[0], foregroundColor[1], foregroundColor[2], backgroundColor[0], backgroundColor[1], backgroundColor[2],
		]);
	...
	createMultipleCharacters(options: MultiCharacterRenderOptions): boolean {
		...
		for (let i = 0; i < text.length; i++) {
			...
			const charVertices = [
				// Triangle 1
				x1, y1, u1, v1, color[0], color[1], color[2], bg[0], bg[1], bg[2],
				x2, y1, u2, v1, color[0], color[1], color[2], bg[0], bg[1], bg[2],
				x1, y2, u1, v2, color[0], color[1], color[2], bg[0], bg[1], bg[2],

				// Triangle 2
				x2, y1, u2, v1, color[0], color[1], color[2], bg[0], bg[1], bg[2],
				x2, y2, u2, v2, color[0], color[1], color[2], bg[0], bg[1], bg[2],
				x1, y2, u1, v2, color[0], color[1], color[2], bg[0], bg[1], bg[2],
			];
