...
export class TextureManager {
	...
	private getTextureFormat(format: string): { format: number; internalFormat: number; type: number } {
		const gl = this.gl;
 ...
 */
export function validateTextureDimensions(width: number, height: number, gl: WebGL2RenderingContext): boolean {
