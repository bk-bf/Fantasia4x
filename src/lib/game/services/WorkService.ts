...
export interface WorkService {
  ...
  getAvailableResourceIdsForWork(
    gameState: GameState,
    workType: string,
    locationId?: string
  ): string[];
...
export class WorkServiceImpl implements WorkService {
  ...
  getAvailableResourceIdsForWork(
    ...
    if (locationId) {
      if (
        this.gameEngine &&
        typeof this.gameEngine.getLocationResourcesForWorkType === 'function'
      ) {
        try {
          const resources = this.gameEngine.getLocationResourcesForWorkType(locationId, workType);
          console.log(
            `[WorkService] Available resources for ${workType} at ${locationId}:`,
            resources
          );
          return resources || [];
        } catch (error) {
      ...
      console.log(
        `[WorkService] No location-specific resources available for ${workType} at ${locationId}`
      );
      return [];
    } else {
  ...
  processWorkHarvesting(gameState: GameState): GameState {
    ...
    gameState.pawns.forEach((pawn) => {
      ...
      console.log(
        `[WorkService] Pawn ${pawn.name} doing ${workType} with priority ${priority} at ${workAssignment.activeLocation}`
      );
      ...
      if (Object.keys(production).length === 0) {
        console.log(
          `[WorkService] No resources available for ${workType} at ${workAssignment.activeLocation}`
        );
      }
      ...
      Object.entries(production).forEach(([resourceId, amount]) => {
        if (amount > 0) {
          ...
          if (actualHarvested > 0) {
            harvestedResources[resourceId] =
              (harvestedResources[resourceId] || 0) + actualHarvested;
            console.log(
              `[WorkService] ${pawn.name} harvested ${actualHarvested} ${resourceId} from ${workAssignment.activeLocation}`
            );
          }
        }
      });
    ...
    const updatedPawns = gameState.pawns.map((pawn) => {
      ...
      if (workAssignment) {
        ...
        if (availableWork && availableWork !== workAssignment.currentWork) {
          console.log(
            `[WorkService] Updating ${pawn.name} work from ${workAssignment.currentWork} to ${availableWork}`
          );
      ...
      if (pawn.state.isWorking !== shouldBeWorking) {
        ...
        console.log(
          `[WorkService] Updated ${pawn.name} working state: ${pawn.state.isWorking} → ${shouldBeWorking} (doing ${workAssignment?.currentWork})`
        );
        return updatedPawn;
      }
  ...
  ensureBasicWorkAssignments(gameState: GameState): GameState {
    ...
    const defaultLocation =
      discoveredLocations.find((loc) => loc.id === 'plains') || discoveredLocations[0];
    ...
    const updatedPawns = gameState.pawns.map((pawn, index) => {
      ...
      if (workAssignment && workAssignment.currentWork) {
        ...
        if (!pawn.state.isEating && !pawn.state.isSleeping) {
          ...
          console.debug(
            `[WorkService] Marked ${pawn.name} as working (${workAssignment.currentWork})`
          );
          return updatedPawn;
        }
      }
  ...
  canPawnDoWorkByType(
    ...
    if (workCategory.toolsRequired && workCategory.toolsRequired.length > 0) {
      ...
      console.log(
        `[WorkService] ${workType} requires tools: ${workCategory.toolsRequired.join(', ')}`
      );
    }
