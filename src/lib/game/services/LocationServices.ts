...
export class LocationServiceImpl implements LocationService {
  ...
  getAvailableResources(locationId: string): string[] {
    ...
    if (!location) {
      ...
      console.log(
        `[LocationService] Available locations:`,
        this.locationsDatabase.map((l) => l.id)
      );
      return [];
    }

    console.log(
      `[LocationService] Found location: ${location.name}, discovered: ${location.discovered}`
    );
    ...
    if (location.resourceNodes) {
      ...
      console.log(
        `[LocationService] Resource nodes for ${locationId}:`,
        Object.keys(location.resourceNodes)
      );
