...
export interface ResearchService {
  ...
  // Calculation Methods
  calculateResearchProgress(researchId: string, gameState: GameState): {
    canStart: boolean;
...
export class ResearchServiceImpl implements ResearchService {
  ...
  canStartResearch(researchId: string, gameState: GameState): boolean {
    ...
    return this.hasPrerequisites(researchId, gameState) &&
  ...
  hasPrerequisites(researchId: string, gameState: GameState): boolean {
    ...
    return research.prerequisites.every(prereq => 
      gameState.completedResearch.includes(prereq)
    );
  }
  ...
  canUnlockWithLore(researchId: string, gameState: GameState): boolean {
    ...
    return gameState.discoveredLore.some(lore => 
  ...
  getApplicableLore(researchId: string): LoreItem[] {
    return LORE_DATABASE.filter(lore => 
      lore.researchUnlocks.includes(researchId)
    );
  }
