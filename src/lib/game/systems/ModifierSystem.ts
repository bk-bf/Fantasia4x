...
export interface ModifierSystem {
	...
	calculateAllWorkEfficiencies(
		pawnId: string,
		gameState: GameState
	): Record<string, WorkEfficiencyResult>;
...
export class ModifierSystemImpl implements ModifierSystem {
		...
		gameState.pawns.forEach((pawn) => {
			WORK_CATEGORIES.forEach((workCategory) => {
				try {
					...
					issues.push(
						`Failed to calculate work efficiency for pawn ${pawn.id}, work ${workCategory.id}: ${error}`
					);
				}
			});
		});
	...
	private calculateStatContribution(
		pawn: Pawn,
		...
		if (workCategory.secondaryStat) {
			...
			sources.push({
				...
				name:
					workCategory.secondaryStat.charAt(0).toUpperCase() + workCategory.secondaryStat.slice(1),
				type: 'stat',
	...
	private calculateTraitWorkBonus(
		pawn: Pawn,
		workType: string
	): { sources: ModifierSource[]; multiplier: number } {
		const sources: ModifierSource[] = [];
	...
	private calculateEquipmentWorkBonus(
		pawn: Pawn,
		workType: string
	): { sources: ModifierSource[]; multiplier: number } {
		const sources: ModifierSource[] = [];
	...
	private calculateBuildingWorkBonus(
		gameState: GameState,
		workType: string,
		locationId?: string
	): { sources: ModifierSource[]; multiplier: number } {
	...
	private calculateResearchWorkBonus(
		gameState: GameState,
		workType: string
	): { sources: ModifierSource[]; multiplier: number } {
		const sources: ModifierSource[] = [];
	...
	private calculatePawnStateModifiers(pawn: Pawn): {
		sources: ModifierSource[];
		multiplier: number;
	} {
		const sources: ModifierSource[] = [];
	...
	/**
	 * Debug method to test efficiency calculations
	 */
	debugWorkEfficiency(pawnId: string, workType: string, gameState: GameState): void {
		console.log(`\n=== WORK EFFICIENCY DEBUG: ${pawnId} doing ${workType} ===`);

		const result = this.calculateWorkEfficiency(pawnId, workType, gameState);

		console.log(`Base Efficiency: ${result.baseValue}`);
		console.log(`Final Efficiency: ${result.totalValue.toFixed(2)}`);
		console.log(`Total Multiplier: ${result.multiplier.toFixed(2)}x`);
		console.log(`\nBreakdown:`);
		console.log(`  Base: ${result.breakdown.base}`);
		console.log(`  Additive: ${result.breakdown.additiveBonus}`);
		console.log(`  Multiplicative: ${result.breakdown.multiplicativeBonus.toFixed(2)}x`);
		console.log(`  Final: ${result.breakdown.final.toFixed(2)}`);

		console.log(`\nSources (${result.sources.length}):`);
		result.sources.forEach((source, index) => {
			console.log(`  ${index + 1}. ${source.name} (${source.type}): ${source.value.toFixed(2)}x - ${source.description}`);
		});

		// Flag potentially overpowered bonuses
		if (result.totalValue > 5.0) {
			console.warn(`⚠️  POTENTIALLY OVERPOWERED: ${result.totalValue.toFixed(2)}x efficiency!`);
		}

		if (result.sources.length > 10) {
			console.warn(`⚠️  TOO MANY SOURCES: ${result.sources.length} modifiers stacking!`);
		}
	}

	/**
	 * Test scenarios with different building/item combinations
	 */
	testBalanceScenarios(gameState: GameState): void {
		console.log('\n=== BALANCE TESTING SCENARIOS ===');

		const testPawn: Pawn | undefined = gameState.pawns[0];
		if (!testPawn) {
			console.log('No pawns available for testing');
			return;
		}

		// Create test states with proper typing
		...
		const oneBuildingState: GameState = {
			...gameState,
			buildingCounts: { workshop: 1 },
			item: []
		};

		const multiBuildingState: GameState = {
			...gameState,
			buildingCounts: { workshop: 5 },
			item: []
		};

		const mixedBuildingState: GameState = {
			...gameState,
			buildingCounts: {
				workshop: 2,
				storage_hut: 3,
		...
		// Scenario 1: No buildings/items
		console.log('\n--- Scenario 1: Baseline (no bonuses) ---');
		const baseline = this.calculateWorkEfficiency(testPawn.id, 'crafting', baselineState);
		console.log(`Baseline crafting efficiency: ${baseline.totalValue.toFixed(2)}x`);

		// Scenario 2: One building
		console.log('\n--- Scenario 2: One Workshop ---');
		const oneBuilding = this.calculateWorkEfficiency(testPawn.id, 'crafting', oneBuildingState);
		const oneIncreasePercent = ((oneBuilding.totalValue / baseline.totalValue - 1) * 100).toFixed(0);
		console.log(`One workshop efficiency: ${oneBuilding.totalValue.toFixed(2)}x (${oneIncreasePercent}% increase)`);

		// Scenario 3: Multiple buildings
		console.log('\n--- Scenario 3: Multiple Workshops ---');
		const multiBuilding = this.calculateWorkEfficiency(testPawn.id, 'crafting', multiBuildingState);
		const multiIncreasePercent = ((multiBuilding.totalValue / baseline.totalValue - 1) * 100).toFixed(0);
		console.log(`Five workshops efficiency: ${multiBuilding.totalValue.toFixed(2)}x (${multiIncreasePercent}% increase)`);

		// Scenario 4: Mix of buildings
		console.log('\n--- Scenario 4: Multiple Building Types ---');
		const mixedBuildings = this.calculateWorkEfficiency(testPawn.id, 'crafting', mixedBuildingState);
		console.log(`Mixed buildings efficiency: ${mixedBuildings.totalValue.toFixed(2)}x`);
	}

	/**
	 * Validate efficiency balance and flag issues
	 */
	validateEfficiencyBalance(gameState: GameState): {
		isBalanced: boolean;
		issues: string[];
		...
		gameState.pawns.forEach((pawn: Pawn) => {
			WORK_CATEGORIES.forEach((work: WorkCategory) => {
				const result = this.calculateWorkEfficiency(pawn.id, work.id, gameState);

				// Flag overpowered efficiencies
				if (result.totalValue > 10.0) {
					issues.push(`CRITICAL: ${pawn.id} has ${result.totalValue.toFixed(1)}x efficiency in ${work.id}`);
					recommendations.push(`Cap multipliers or reduce building bonuses for ${work.id}`);
				} else if (result.totalValue > 5.0) {
					issues.push(`WARNING: ${pawn.id} has ${result.totalValue.toFixed(1)}x efficiency in ${work.id}`);
				}

				// Flag too many stacking sources
				if (result.sources.length > 8) {
					issues.push(`Too many bonuses stacking for ${pawn.id} in ${work.id}: ${result.sources.length} sources`);
					recommendations.push(`Consider consolidating or capping bonus sources`);
				}

				// Flag multiplicative explosion
				if (result.multiplier > 10.0) {
					issues.push(`Multiplicative explosion: ${result.multiplier.toFixed(1)}x total multiplier for ${pawn.id} in ${work.id}`);
					recommendations.push(`Use additive bonuses instead of multiplicative for some sources`);
				}
			});
		});
