...
export class ModifierSystemImpl implements ModifierSystem {
  ...
  private calculatePawnStateModifiers(pawn: Pawn): {
    ...
    for (const condition of pawn.conditions ?? []) {
      ...
      if (
        activeStage.modifiers.workEfficiency !== undefined &&
        activeStage.modifiers.workEfficiency < 1
      ) {
  ...
  debugWorkEfficiency(pawnId: string, workType: string, gameState: GameState): void {
    ...
    result.sources.forEach((source, index) => {
      console.log(
        `  ${index + 1}. ${source.name} (${source.type}): ${source.value.toFixed(2)}x - ${source.description}`
      );
    });
  ...
  testBalanceScenarios(gameState: GameState): void {
    ...
    const oneIncreasePercent = ((oneBuilding.totalValue / baseline.totalValue - 1) * 100).toFixed(
      0
    );
    console.log(
      `One workshop efficiency: ${oneBuilding.totalValue.toFixed(2)}x (${oneIncreasePercent}% increase)`
    );
    ...
    const multiIncreasePercent = (
      (multiBuilding.totalValue / baseline.totalValue - 1) *
      100
    ).toFixed(0);
    console.log(
      `Five workshops efficiency: ${multiBuilding.totalValue.toFixed(2)}x (${multiIncreasePercent}% increase)`
    );

    // Scenario 4: Mix of buildings
    console.log('\n--- Scenario 4: Multiple Building Types ---');
    const mixedBuildings = this.calculateWorkEfficiency(
      testPawn.id,
      'crafting',
      mixedBuildingState
    );
    ...
    gameState.pawns.forEach((pawn: Pawn) => {
      WORK_CATEGORIES.forEach((work: WorkCategory) => {
        ...
        if (result.totalValue > 10.0) {
          issues.push(
            `CRITICAL: ${pawn.id} has ${result.totalValue.toFixed(1)}x efficiency in ${work.id}`
          );
          recommendations.push(`Cap multipliers or reduce building bonuses for ${work.id}`);
        } else if (result.totalValue > 5.0) {
          issues.push(
            `WARNING: ${pawn.id} has ${result.totalValue.toFixed(1)}x efficiency in ${work.id}`
          );
        }
        ...
        if (result.sources.length > 8) {
          issues.push(
            `Too many bonuses stacking for ${pawn.id} in ${work.id}: ${result.sources.length} sources`
          );
        ...
        if (result.multiplier > 10.0) {
          issues.push(
            `Multiplicative explosion: ${result.multiplier.toFixed(1)}x total multiplier for ${pawn.id} in ${work.id}`
          );
