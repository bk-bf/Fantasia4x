...
const createMockGameState = (): GameState => ({
    ...
    item: [
        ...
        {
            id: 'stone_axe', name: 'Stone Axe', amount: 1, type: 'tool', category: 'harvesting',
            effects: { woodcuttingBonus: 0.3 }, workTypes: ['woodcutting']
        }
    ],
    worldMap: [],
    discoveredLocations: [
...
function testCalculateBuildingEffects() {
    ...
    const result = modifierSystem.calculateBuildingEffects(
        'simple_workshop',
        mockGameState
    );
...
function testCaching() {
    ...
    // Test cache clearing
    modifierSystem.clearCache();
    const result3 = modifierSystem.calculateWorkEfficiency('test_pawn_1', 'woodcutting', mockGameState);
