...
import { DEFAULT_SERVICE_INTEGRATION_CONFIG } from './ServiceIntegration';
...
export class ServiceIntegrationFactory {
  ...
  createServiceRegistry(): ServiceRegistry {
    ...
    return new ServiceRegistryImpl(lifecycleManager, container, this.config);
  }
  ...
  private registerCoreServices(container: ServiceContainer): void {
    ...
    container.register('buildingService', () => new BuildingServiceImpl(), []);

    // WorkService - no dependencies (uses ItemService internally)
    container.register('workService', () => new WorkServiceImpl(), []);

    // ResearchService - no dependencies (uses ItemService internally)
    container.register('researchService', () => new ResearchServiceImpl(), []);

    // PawnService - placeholder implementation
    container.register('pawnService', () => new PawnServiceImpl(), []);

    // EventService - placeholder implementation
    container.register('eventService', () => new EventServiceImpl(), []);
  }
...
class ServiceCommunicationCoordinatorImpl implements ServiceCommunicationCoordinator {
  ...
  async sendMessage(message: any): Promise<any> {
    ...
    throw new Error(
      `No handler found for message type ${message.type} on service ${message.target}`
    );
  }
...
class GameEngineServiceIntegrationImpl implements GameEngineServiceIntegration {
  ...
  validateServiceIntegration(): any {
    ...
    return {
      isHealthy:
        dependencyValidation.isValid && Object.values(serviceStatuses).every((s) => s.isHealthy),
      serviceStatuses,
      communicationHealth: {
...
class PawnServiceImpl {
  ...
  processPawnNeeds(): any {
    return {};
  }
  calculateEffectiveStats(): any {
    return {};
  }
...
class GameEngineImpl implements GameEngine {
  ...
  calculatePawnEfficiency(): number {
    return 1.0;
  }
  calculateBuildingEffects(): any {
    return {};
  }
  calculateCraftingTime(): number {
    return 1;
  }
  calculateResourceProduction(): Record<string, number> {
    return {};
  }
  calculateCombatEffectiveness(): number {
    return 1.0;
  }
  ...
  integrateServices(services: ServiceRegistry): void {
    ...
    if (this.gameState && !this.interactionCoordinator) {
      this.interactionCoordinator = new SystemInteractionCoordinatorImpl(services, this.gameState);
    }
  }
  ...
  initialize(initialState: GameState, services: ServiceRegistry): any {
    ...
    this.interactionCoordinator = new SystemInteractionCoordinatorImpl(services, initialState);
...
export async function createIntegratedGameSystem(
  ...
  return ServiceIntegrationFactory.getInstance(config).createIntegratedSystem(
    initialGameState,
    gameEngineConfig
  );
}
