...
function generateImplications(
  ...
  Object.keys(IMPLICATION_DATABASE).forEach((category) => {
    const categoryImplications =
      IMPLICATION_DATABASE[category as keyof typeof IMPLICATION_DATABASE];
    implications[category] =
      categoryImplications[Math.floor(Math.random() * categoryImplications.length)];
  });
