...
import { recomputeWound } from '../../defs/wounds';
...
function pickFlaw(ids: string[], rand: () => number): Trait | undefined {
  const pool = ids.map((id) => getTraitById(id)).filter((t): t is Trait => !!t);
...
export function resolveTraitGamble(
  ...
  if (outcome !== 'bad' && spec.traitPool.length) {
    ...
    trait = getTraitById(spec.traitPool[idx]);
  }
  const flaw =
    outcome === 'good'
      ? undefined
...
export function lineageGrowthEvent(
  ...
  if (full && !pawnLineage(pawn)) {
    const parentId = lineageDef(full.lineage)?.parent;
    const parent = parentId ? getTraitById(parentId) : undefined;
    if (parent) {
      const added: string[] = [];
  ...
  if (rng.random() < EVOLVE_CHANCE) {
    const staged = (pawn.traits ?? []).find((t) => t.evolvesTo && getTraitById(t.evolvesTo));
    if (staged) {
      const next = getTraitById(staged.evolvesTo as string) as Trait;
