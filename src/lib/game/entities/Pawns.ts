...
function addWorkEfficiencies(
  ...
  workTypes.forEach((workType) => {
    ...
    // Trait bonuses
    const traitBonus =
      abilities[`workEfficiency_${workType}`]?.value || abilities[`workEfficiency_all`]?.value;
    if (traitBonus) {
    ...
    const toolBonus = abilities[`${workType}Bonus`]?.value || abilities.toolEfficiency?.value;
    if (toolBonus) {
      efficiency *= 1 + toolBonus;
      calculation += ` × tool bonus (1 + ${toolBonus})`;
    }

    addAbility(abilities, `${workType}Efficiency`, Math.round(efficiency * 100) / 100, calculation);
  });
}
  ...
  Object.keys(abilities).forEach((abilityName) => {
    ...
    if (
      ...
      (lowerName.includes('damage') &&
        (lowerName.includes('attack') || lowerName.includes('combat')))
    ) {
    ...
    // Survival abilities - MUCH more comprehensive
      ...
      (lowerName.includes('fire') &&
        (lowerName.includes('resist') || lowerName.includes('immun'))) ||
      (lowerName.includes('cold') &&
        (lowerName.includes('resist') ||
          lowerName.includes('immun') ||
    ...
    // Mental abilities
      ...
      (lowerName.includes('group') &&
        (lowerName.includes('bonus') || lowerName.includes('penalty'))) ||
