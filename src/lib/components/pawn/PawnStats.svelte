<script lang="ts">
  ...
  function calculateAllModifierResults(pawn: Pawn, gameState: GameState) {
    ...
    capacityStats.forEach((st) => {
      const cap = calculateCapacity(pawn, st.id, st.id === 'sight' ? lightMult : undefined);
      ...
      const adjusted: typeof baseEff = {
        ...
        sources: [
          ...
          },
          {
            ...
            description: `Sight capacity × ${(capacities.sight ?? 1).toFixed(2)} (light × ${lightMult.toFixed(2)})`
          }
        ]
      };
  ...
  // ── Formula evaluator: delegates to PawnStatService ──
  function evaluateFormula(
    ...
    // Reconstruct a minimal formula string that PawnStatService can evaluate
    // by substituting capacities back into the formula
    let expr = formula
      .replace(/×/g, '*')
      .replace(/−/g, '-')
  ...
  function calculateCapacity(
    ...
    const descriptions: Record<string, string> = {
      ...
      pain: 'Injuries + Limb damage + Bleeding',
      manipulation: 'Arms: weaker × 0.3 + avg × 0.7',
      sight: 'Eyes: weaker × 0.4 + avg × 0.6 + 0.05',
      moving: 'Legs: weaker × 0.5 + avg × 0.5',
      blood_pumping: 'Heart × 0.9 + 0.10 baseline',
      blood_filtration: 'Kidneys: weaker × 0.4 + avg × 0.6',
      breathing: 'Lungs: weaker × 0.5 + avg × 0.5 + 0.05',
      digestion: 'Stomach × 0.6 + Liver × 0.4',
      talking: 'Jaw × 0.9 + 0.10 baseline',
      hearing: 'Ears: weaker × 0.3 + avg × 0.7 + 0.15'
    };
    const desc =
      capacityId === 'sight' && lightMultiplier !== undefined
        ? `${descriptions.sight} · light × ${lightMultiplier.toFixed(2)}`
        : descriptions[capacityId] || 'Body capacity';
    const sources: ModifierSource[] = [
      {
        id: capacityId,
        ...
        {#each Object.entries(allModifierResults.resistanceBonus) as [resType, result]}
          <div class="ability-card" data-category="resistance">
            ...
            <div class="ability-calculation">
              <div class="calculation-header">
                <span>Modifiers ({result.sources.length})</span>
                <button class="toggle-breakdown" on:click={() => toggleBreakdown(`res_${resType}`)}>
                  ...
                  {#each result.sources as source}
                    ...
                    >
                      <span class="source-label"
                        >{source.name}: {(source.value * 100).toFixed(1)}%</span
                      >
                      <div class="source-description">{source.description}</div>
                    </div>
                  {/each}
          ...
          <div class="ability-card" data-category="capacity">
            <div class="ability-header">
              ...
              <span
                ...
                style="color: {result.totalValue >= 0.8
                  ? 'var(--positive)'
            ...
            <div class="ability-calculation">
              <div class="calculation-header">
                ...
                <button class="toggle-breakdown" on:click={() => toggleBreakdown(`cap_${capType}`)}>
