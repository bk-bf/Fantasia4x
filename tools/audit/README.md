...
| `playtest` | A pull request labelled `needs playtest`, and the card goes straight to `Needs Playtest`. The worktree is kept, and it is given a free port in `.devport` so `./dev.sh` inside it runs beside the checkout's own dev server instead of fighting it for 5173. |

The card moves `Ready → In progress → PR ready`, or `Ready → In progress → Needs Playtest` on the
`playtest` route, and stays there until the pull request merges. An
