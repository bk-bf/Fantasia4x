...
const FAMILY_KIND = {
  contract: 'drift', boundary: 'boundary', 'silent-failure': 'correctness',
  units: 'correctness', 'hot-path': 'performance', tests: 'test-gap',
  reachability: 'correctness', data: 'data', 'single-source': 'drift'
};
const FAMILY_SEVERITY = {
  contract: 'high', boundary: 'medium', 'silent-failure': 'high',
  units: 'high', 'hot-path': 'medium', tests: 'low',
  reachability: 'low', data: 'medium', 'single-source': 'high'
};
...
function groupOf(file) {
  const parts = file.replace(/^src\/lib\//, '').replace(/^src\//, '').split('/');
...
export function groupFindings(db) {
  const rows = db.prepare(`
    SELECT f.id, f.symbol_key, f.rule_id, f.summary, f.evidence, f.state,
     ...
     WHERE f.state = 'open'
     ORDER BY f.rule_id, s.file, s.start_line`).all();
  ...
  for (const r of rows) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        rule_id: r.rule_id, family: r.family, rule_title: r.rule_title,
        authority: r.authority, group: g, findings: []
      });
    }
...
function renderBody(g, { existingGithub } = {}) {
  ...
  lines.push('');
  lines.push(`> **Related:** [issues/README](README.md) · [tools/audit](../../tools/audit/README.md)` +
             (g.authority ? ` · [rule source](../../${g.authority.split('#')[0]})` : ''));
  lines.push('');

  lines.push('## What breaks');
  lines.push('');
  lines.push(`Rule \`${g.rule_id}\` — ${g.rule_title} — holds in ${n} ` +
  ...
  if (rest > 0) {
    lines.push('');
    lines.push(`…and ${rest} more under the same rule. \`node tools/audit/audit.mjs findings\` lists them all.`);
  }
  lines.push('');

  lines.push('## Why nothing caught it');
  lines.push('');
  lines.push(`Nothing below the judgment tier can decide this one: it is why \`${g.rule_id}\` ` +
  ...
  lines.push(`- [ ] Add the check that would have caught it, or record why it stays a judgment call.`);
  lines.push(`- [ ] \`pnpm check\` and \`pnpm test:related\` on the changed files are green.`);
  lines.push('');

  lines.push('## Out of scope');
  lines.push('');
  lines.push(`Sites outside \`${g.group}\`, and any other rule's findings — they are their own ` +
...
export function upsertIssue(root, g, rulesById) {
  ...
  if (existsSync(path)) {
    ...
    patchIssue(path, {
      title: titleFor(g), kind, severity, files, symbols,
      rules: [g.rule_id], updated: today()
    });
    if (changed) {
  ...
  writeIssue(root, {
    data: {
      id, title: titleFor(g), status: 'open', kind, severity,
      ready: false, origin: 'audit', rules: [g.rule_id],
      files, symbols, created: today(), updated: today()
    },
    body: renderBody(g)
  });
